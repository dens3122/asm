...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/05/27 15:10:02 by jritchie          #+#    #+#             */
/*   Updated: 2019/06/15 16:44:32 by jritchie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		if (line[i] == '"')
		{
			...
			else
			{
				...
				if (!check_simple_comment(line + i + 1))
					error_clear_exit(ch, "Error", 1, line);
				break ;
			}
		}
...
int		check_name(t_asm *ch, char *line)
{
	...
	if (ft_strncmp(line, NAME_CMD_STRING, i) == 0)
	{
		...
		if (ch->name_init == 0)
			error_clear_exit(ch, "Wrong name", 1, line);
		else
			ch->name[start] = '\0';
		return (1);
	}
